...
const Button = ({ className, textClassName, title, onPress, disabled=false } : ButtonProps) => {

    if (disabled) {
        return (
            <View style={{flex: 1, padding: 0.5}}>
                <View className={twMerge("bg-slate-800 p-2 rounded-full flex flex-row items-center justify-center min-h-[4vh]", className)}>
