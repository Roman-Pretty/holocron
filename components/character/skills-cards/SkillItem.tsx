...
const DicePool = ({
  ...
  return (
    <>
      <Text className="font-[swrpg] text-yellow-300 text-xs">
        {Array.from({ length: diePool[0] }).map((_, i) => "c")}
      </Text>
      <Text className="font-[swrpg] text-green-600 text-xs">
  ...
  return (
    <View className={`${index % 2 === 1 ? " " : "bg-black/20"} px-2 flex-row`}>
      <View className="flex flex-row justify-between w-full items-center">
        <Text className="text-white text-sm font-semibold capitalize] w-5/12">
